...
while(flag){
{printf("%s","Inserisci l'operazione da effettuare (somma, sottrazione, divisione, moltiplicazione, tutteleoperazioni): ");buffer = (char*) malloc((1024*5)*sizeof(char) );
scanf("%[^\n]", buffer);
operazione= (char*) malloc( (strlen(buffer) + 1) *sizeof(char) );
strcpy(operazione,buffer);
 free(buffer);
getchar();printf("%s","Inserisci il primo input: ");scanf("%lf", &input1);getchar();printf("%s","Inserisci il secondo input: ");scanf("%lf", &input2);getchar();printf("%s%s%s%f%s%f\n","Hai scelto l'operazione ",operazione," con gli argomenti ",input1," e ",input2);if(strcmp( "somma", operazione) == 0){somma_fun(input1, input2, &result);
...
}else{printf("%s\n","Operazione non consentita");}}}}}if(strcmp( "tutteleoperazioni", operazione) != 0){printf("%s%f\n","Il risultato è: ",result);}else{printf("%s\n",stampa_fun(string_concat(string_concat(string_concat(string_concat(string_concat(string_concat(string_concat(string_concat("I risultati delle 4 operazioni sono \n", float2str(res1)), "\n"), float2str(res2)), "\n"), float2str(res3)), "\n"), float2str(res4)), "\n")));}printf("%s","Vuoi continuare? (1 yes/0 no): ");scanf("%lf", &answer);getchar();if(answer == 1){flag = 1;
}else{flag = 0;
}}
}
